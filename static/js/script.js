...
function close_shop_item() {
    ...
    $(".top-shop").hover(function () {
        $.ajax({
            url: "http://localhost/FaceAlbum/index.php/friend/get_notifications",
            type: "GET",
            dataType: "json",
            success: function (response) {
                $(response['notifications']).each(function (index, element) {
                    $(".top-shop aside").find("p").remove();
                    $(".top-shop aside").find(".item-top-sellers").remove();
                    var notifications = "<p>There Are <span>"+response['notifications'].length+"</span> new notifications</p>";
                    if (index < 3) {
                        notifications += "<div class='item-top-sellers clearfix'><img src='static/pic/shop/70x80/1.jpg' alt>";
                        notifications += "<h3 class='title'><a href='http://localhost/FaceAlbum/index.php/album/others_album/"+element.username+"'>"+ element.username+ "</a> <a class='shop-close'><i class='fa fa-close'></i></a></h3>";
                        notifications += "<div class='old-price'>recent news:<span class='price'>"+ element.news_num+"</span></div>";
                        notifications += "<div class='old-price'>recent photos:<span class='price'>"+ element.photo_num+"</span></div>";
                        notifications += "<div class='cws_divider mt-10 mb-10'> </div>";
                    }
                    $(".top-shop .shop-cart-menu").prepend(notifications);
                });
            }
        });
    });
